...
void onModeToggle() {
  // Toggle between APP and TRMNL modes
  if (SETTINGS.deviceMode == CrossPointSettings::MODE_APP) {
    SETTINGS.deviceMode = CrossPointSettings::MODE_TRMNL;
  } else {
    SETTINGS.deviceMode = CrossPointSettings::MODE_APP;
  }
  SETTINGS.saveToFile();

  // Always go back to home - the mode indicator will show current mode
  // TRMNL mode functionality will be added later
  onGoHome();
}

void onGoHome() {
  // Clear the open epub path when going home so we return here on wake
  ...
  enterNewActivity(new HomeActivity(
      ...
      onGoToSettings, onGoToFileTransfer, onGoToBrowser, onModeToggle));
}
...
void setup() {
  ...
  // Check boot behavior setting
  if (SETTINGS.bootBehavior == CrossPointSettings::BOOT_HOME ||
      APP_STATE.openEpubPath.empty()) {
    onGoHome();
  } else {
    // BOOT_RESUME and we have a saved book path
